    ...
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        dispatcher.register(literal("locki")
                ...
                .then(literal("lock")
                        ...
                        .requires(Permissions.require("locki.command.lock.self", 2))
                        .then(argument("node", InventoryNodeArgumentType.inventoryNode())
                                .executes(ctx -> lock(ctx.getSource(), singleton(ctx.getSource().getPlayer()), InventoryNodeArgumentType.getInventoryNode(ctx, "node"), COMMAND_LOCK))
                                .then(argument("targets", players())
                                        // Require another permission for disguising other people
                                        .requires(Permissions.require("locki.command.lock", 2))
                                        .executes(ctx -> lock(ctx.getSource(), getPlayers(ctx, "targets"), InventoryNodeArgumentType.getInventoryNode(ctx, "node"), COMMAND_LOCK))
                                        .then(argument("lock", inventoryLock())
                                                .executes(ctx -> lock(ctx.getSource(), getPlayers(ctx, "targets"), InventoryNodeArgumentType.getInventoryNode(ctx, "node"), getInventoryLock(ctx, "lock")))
                                        )
                                )
                        )
                )
                .then(literal("unlock")
                        ...
                        .requires(Permissions.require("locki.command.lock.self", 2))
                        .then(argument("node", InventoryNodeArgumentType.inventoryNode())
                                .executes(ctx -> unlock(ctx.getSource(), singleton(ctx.getSource().getPlayer()), InventoryNodeArgumentType.getInventoryNode(ctx, "node"), COMMAND_LOCK))
                                .then(argument("targets", players())
                                        // Require another permission for disguising other people
                                        .requires(Permissions.require("locki.command.lock", 2))
                                        .executes(ctx -> unlock(ctx.getSource(), getPlayers(ctx, "targets"), InventoryNodeArgumentType.getInventoryNode(ctx, "node"), COMMAND_LOCK))
                                        .then(argument("lock", inventoryLock())
                                                .executes(ctx -> unlock(ctx.getSource(), getPlayers(ctx, "targets"), InventoryNodeArgumentType.getInventoryNode(ctx, "node"), getInventoryLock(ctx, "lock")))
                                        )
                                )
                        )
                )
